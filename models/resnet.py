#modified pytorch default resnet code: https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py
import torch
...
class GateModule(nn.Module):
    def __init__(self):
        ...
        self.conv_1 = nn.Conv2d(192,192,1,1,0)
        self.conv_2 = nn.Conv2d(192,192*2,1,1,0,groups=192)
        self.batch_norm = nn.BatchNorm2d(192)
        self.relu = nn.ReLU()
    def forward(self,x):
        ch = self.adaptive_pool(x)
        ch = self.conv_1(ch)
        ch = self.batch_norm(ch)
        ch = self.relu(ch)
        ch = self.conv_2(ch)
...
class dct_resnet(nn.Module):
    def __init__(self):
        super(dct_resnet,self).__init__()
